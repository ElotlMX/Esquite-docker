...
In version 0.2, a control script provided multiple configuration options. While useful, it was also prone to errors and somewhat cumbersome.

This new version works exclusively with a pre-configured Docker Compose setup, reducing the number of manual configurations required.
...
If no custom Docker network configuration was used, check the container logs to find the IP address:
...
Alternatively, use the `inspect` command to retrieve the container's IP address:
...
---
#### Access to Corpus Admin section
The management section is available on http://[IP-OR-HOSTNAME]/corpus-admin
...
- NGINX is configured as a reverse proxy for handling all HTTP traffic for Django backend. This ensures easier deployment.
...
Alternatively, if you plan to use this Elasticsearch container in a production environment, ensure you fine-tune its parameters accordingly (e.g., index name, memory limits, API permissions, etc.).
